...
class Event extends Bookable
{
    ...
    public function search($request)
    {
        ...
        {
            $query->join('bravo_event_term as tt1', function($join) use ($term_id){
                $join->on('tt1.target_id', "bravo_events.id");
    ...
    public function savePassengers(Booking $booking, Request $request)
    {
        $maps = $this->ticketTypesToGuestMap($booking->getJsonMeta('ticket_types'));
    ...
    public function ticketTypesToGuestMap($ticket_types)
    {
        if (empty($ticket_types) or !is_array($ticket_types)) return [];
        $res = [];
        foreach ($ticket_types as $ticket_type) {
            for ($i = 1; $i <= $ticket_type['number']; $i++) {
                $res[] = $ticket_type;
            }
        }
        return $res;

<tr>
    ...
    <td width="2%">
        ...
        @if(!empty(setting_item("flight_allow_vendor_can_change_their_booking_status")))
            ...
            <div class="dropdown-menu">
                @if(!empty($statues))
                    @foreach($statues as $status)
                        <a href="{{ route("flight.vendor.booking_report.bulk_edit" , ['id'=>$booking->id , 'status'=>$status]) }}">
        ...
        @endif
            @if(!empty(setting_item("flight_allow_vendor_can_change_paid_amount")))
                ...
                @includeIf ($service->set_paid_modal_file ?? '')
            @endif
    </td>
</tr>
